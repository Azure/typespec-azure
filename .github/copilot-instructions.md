...
- By default, when pushing changes and creating pull requests, push to your personal fork and open PRs against the main Azure fork's `main` branch.
- LintDiff workflow exception: base development worktrees, branches, and PRs on `Azure/typespec-azure`'s `feature/lintdiff-migration-new` branch. Promotion worktrees, branches, and PRs use `Azure/typespec-azure`'s `main` branch. New development, promotion, and workflow-instruction source branches belong to `Azure/typespec-azure`; instruction-only PRs target `feature/lintdiff-migration-new`.
- Existing task-owned LintDiff PRs or worktrees may retain personal-fork source branches with explicit task-scoped authorization and verified repository, branch, ownership, permissions, and current state. Creating the first development PR from an existing fork-backed worktree requires the separate [existing-worktree publication authorization](skills/shared/recovery-context.md#existing-fork-worktree-publication). Neither exception authorizes replacing, retargeting, or closing existing PRs, successor fork PRs, or instruction-only fork PRs.
