...
Extended rule docs live beside their implementations as `packages/typespec-azure-resource-manager/src/rules/<rule-name>.md` and are referenced from the rule's `docs` field. Their frontmatter `title` should match the Markdown file name because the generated URL is derived from that name. The generated "Full name" block still shows the fully qualified registered rule name, which may include an `arm-` prefix.

Rule pages under `website/src/content/docs/docs/libraries/azure-resource-manager/rules/` are generated and ignored by git. Audit the source Markdown and generated output instead of editing those pages directly. The output must have valid frontmatter (`title`, `sourceUrl`), accurate messages, and exactly one page per registered rule.

## Reference Documentation Generation

- `pnpm regen-docs` from `packages/typespec-azure-resource-manager/` generates the `reference/` files and website rule pages through `tspd`.
...
- Keep the formatter's multiline layout for long generic aliases in examples, such as `AgentConversation<...>`; a formatting-only change can be the required correction.
...
- Two deployment variants differ by property visibility and ownership: **Appliance** (service-owned, read-only) and **Platform** (client-owned, writable; `baseTypes` always read-only). Models: `AgentDefinitionAppliance<HasInstructions>`/`AgentDefinitionPlatform<HasModelDeploymentRef, HasInstructions>` (boolean value params gate the optional properties), `AgentPropertiesAppliance`/`AgentPropertiesPlatform<AgentDefinitionType>`, `AgentToolTypeAppliance`/`AgentToolTypePlatform`. `modelDeploymentRef` exists only in the Platform variant; the Appliance variant has no such property and `AgentPropertiesAppliance.definition` is `@visibility(Lifecycle.Read)`.
...
- `Relationship<Properties>` is an `ExtensionResource` template that applies `@azureBaseType` with `BaseType.Relationship` and contract version `2026-04-01`.
- Its properties model must extend `RelationshipProperties`, which provides ARM-managed `baseTypes`, source and target IDs and tenant IDs, and provisioning state.
- The canonical operations use `Extension.Read`, `Extension.CreateOrReplaceAsync`, `Extension.CustomPatchAsync`, `Extension.DeleteWithoutOkAsync`, and `Extension.ListByTarget`.
- Canonical sample: `packages/samples/specs/resource-manager/resource-types/relationship/main.tsp`.

## Billing Data Placement

`BillingDataProperty` belongs in a resource-specific properties model, not in the standard ARM resource envelope. The public model documentation and canonical billing-data sample both use this placement.
