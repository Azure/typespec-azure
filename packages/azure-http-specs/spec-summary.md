...
### Client_ResponseAsBool_exists

- Endpoint: `head /client/response-as-bool/exists`

Testing that a HEAD operation decorated with `@responseAsBool` returns `true` when the resource exists (2xx response).
The generated SDK should return `true` for this operation.

Expected response status: 200
Expected SDK return value: true

### Client_ResponseAsBool_notExists

- Endpoint: `head /client/response-as-bool/not-exists`

Testing that a HEAD operation decorated with `@responseAsBool` returns `false` when the resource does not exist (404 response).
The generated SDK should return `false` for this operation instead of raising an error.

Expected response status: 404
Expected SDK return value: false

### Client_Scope_create

- Endpoint: `post /client/scope/scoped`

Testing that operations scoped to specific languages are only generated for those languages.
The `create` operation is scoped to `python` and `java` only.
SDKs for other languages (C#, TypeScript, Go) should NOT generate this operation.
...
- Endpoint: `get /client/scope/all`

Testing that operations scoped to all languages are generated.
The `list` operation has no `@scope` decorator and should be generated for all languages.
