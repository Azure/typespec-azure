...
`;
const code = "@azure-tools/typespec-azure-resource-manager/no-unsafe-patch-body-properties";
  ...
  it("reports the documented unsafe standard-template payload", async () => {
    ...
    await documentationTester
      ...
      .toEmitDiagnostics([
        ...
        missing("properties.extra"),
        defaultValue("properties.enabled"),
        missing("properties.enabled"),
      ]);
  });
...
describe("scope and registration", () => {
  ...
  describe("native scope independent of emitter settings", () => {
    it.each([
      [
        "property",
        "model Patch { extra?: string; } @patch op update(@body body: Patch): { value: string };",
      ],
      ["operation", "@patch op update(@body body: { extra?: string }): { value: string };"],
      [
        "GET fallback",
        '@route("/widgets") @patch op update(@body body: { extra?: string }): AcceptedResponse; @route("/widgets") @get op read(): { value: string };',
      ],
    ])("checks native %s without emitter configuration", async (_, snippet) => {
      await tester.expect(`${service}${snippet}`).toEmitDiagnostics(missing("extra"));
    });
    it("uses matching native response properties without emitter configuration", async () => {
      await tester
        .expect(
          `${service}
        model Resource { value: string; }
  ...
  it("registers only the combined incoming rule and retains existing PATCH rules", () => {
    const names = $linter.rules.map((r) => r.name);
    expect(names).toContain("no-unsafe-patch-body-properties");
    expect(names).toContain("arm-resource-patch");
    expect(names).toContain("patch-envelope");
    ...
    expect(names).not.toContain("no-unsupported-patch-properties");
  });
});
