...
export const consistentPatchPropertiesRule = createRule({
  ...
  docs: fileRef.fromPackageRoot("src/rules/consistent-patch-properties.md"),
  description:
    "ARM PATCH body properties must exist in the resource model at the same nesting level.",
  severity: "warning",
  url: "https://azure.github.io/typespec-azure/docs/libraries/azure-resource-manager/rules/consistent-patch-properties",
  messages: {
        ...
        for (const service of services) {
          for (const httpOperation of service.operations) {
            if (
              ...
              isInternalTypeSpec(context.program, httpOperation.operation)
            ) {
              continue;
            }
