# Should generate test for long running operation with flattened properties

Test generation should flatten @flattenProperty decorated model properties in both input and response assertions.
...
@@Azure.ClientGenerator.Core.Legacy.flattenProperty(Employee.properties);
...
{
  "title": "Employees_CreateOrUpdate_Flatten",
...
describe("create a Employee", () => {
  ...
  it("should create a Employee for employeesCreateOrUpdateFlatten", async function () {
    ...
    assert.strictEqual(result.city, "Seattle");
