...
import { generateAssertionsForValue } from "../../src/modular/helpers/example-value-helpers.js";
...
describe("exampleGeneration", () => {
  describe("generateAssertionsForValue", () => {
    it("generates assertions for primitive types", () => {
      ...
      assert.deepEqual(stringAssertions, ['assert.strictEqual(result.name, "hello");']);
      assert.deepEqual(booleanAssertions, ["assert.strictEqual(result.enabled, true);"]);
      assert.deepEqual(numberAssertions, ["assert.strictEqual(result.count, 42);"]);
      assert.deepEqual(unknownAssertions, ["assert.isDefined(result.payload);"]);
    ...
    it("generates assertions for utcDateTime and bytes", () => {
      ...
      assert.deepEqual(Buffer.from("YWJj", "base64"), Buffer.from("YWJj", "base64"));
    });
      ...
      const assertions = generateAssertionsForValue(
        {
          ...
          type: {
            kind: "model",
            properties: [{ kind: "property", name: "systemData", flatten: false }],
          },
        } as any,
        "result",
      );

      assert.deepEqual(assertions, ['assert.strictEqual(result.systemData?.createdBy, "admin");']);
    });
      ...
      const assertions = generateAssertionsForValue(
        {
          ...
          type: {
            kind: "model",
            properties: [{ kind: "property", name: "properties", flatten: true }],
          },
        } as any,
        "result",
      );

      assert.deepEqual(assertions, ['assert.strictEqual(result.displayName, "demo");']);
    });
    ...
    it("generates assertions for null and union values", () => {
      ...
      assert.deepEqual(nullAssertions, ["assert.strictEqual(result.value, null);"]);
      assert.strictEqual(null, null);
      assert.deepEqual(unionAssertions, ["assert.strictEqual(result.unionValue, 7);"]);
    });
