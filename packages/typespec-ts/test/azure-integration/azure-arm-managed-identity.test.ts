  ...
  describe("Error Handling", () => {
    ...
    it("should handle user-defined error for bad request (400)", async () => {
      ...
      if (result.status === "400") {
        ...
        assert.strictEqual(result.body.error?.message, "Username should not contain only numbers.");
        assert.isObject(result.body.error?.innererror);
        assert.strictEqual(result.body.error?.innererror?.exceptiontype, "general");
      }
    });
  });
});
