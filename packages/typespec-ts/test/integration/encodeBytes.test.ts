...
import EncodeBytesClientFactory, { BytesClient } from "./generated/encode/bytes/src/index.js";
...
describe("EncodeDatetimeClient Rest Client", () => {
  ...
  describe("query", () => {
    ...
    it(`should get bytes base64url-array`, async () => {
      const result = await client.path(`/encode/bytes/query/base64url-array`).get({
        queryParameters: {
          value: ["dGVzdA", "dGVzdA"],
        },
      });
  ...
  describe("property", () => {
    ...
    it(`should post bytes base64url array`, async () => {
      const result = await client.path(`/encode/bytes/property/base64url-array`).post({
        body: {
          value: ["dGVzdA", "dGVzdA"],
        },
      });
  ...
  describe("header", () => {
    ...
    it(`should get bytes  base64url-array`, async () => {
      const result = await client.path(`/encode/bytes/header/base64url-array`).get({
        headers: {
  ...
  describe("request body", () => {
    ...
    it(`should post bytes with custom content type`, { timeout: 10000 }, async () => {
      const result = await client.path(`/encode/bytes/body/request/custom-content-type`).post({
  ...
  describe("response body", () => {
    ...
    it(`should get bytes with custom content type`, async () => {
      const result = await client.path(`/encode/bytes/body/response/custom-content-type`).get({
        contentType: "image/png",
      });
      assert.strictEqual(result.status, "200");
      assert.strictEqual(uint8ArrayToString(result.body, "utf-8"), pngFile);
    ...
    it(`should get bytes with octet-stream content type`, async () => {
      const result = await client.path(`/encode/bytes/body/response/octet-stream`).get({
        contentType: "application/octet-stream",
      });
      assert.strictEqual(result.status, "200");
      assert.strictEqual(uint8ArrayToString(result.body, "utf-8"), pngFile);
