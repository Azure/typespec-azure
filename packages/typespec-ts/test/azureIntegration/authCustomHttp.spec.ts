...
describe("AuthHttpCustomClient Rest Client", () => {
  ...
  it("should return 204 when the apiKey is valid", async () => {
    const result = await validKeyClient.path("/authentication/http/custom/valid").get();
    assert.strictEqual(result.status, "204");
  });

  it("should return 403 when the apiKey is invalid", async () => {
    const result = await invalidKeyClient.path("/authentication/http/custom/invalid").get();
