...
beforeEach(async () => {
  ...
  tester = createLinterRuleTester(runner, noRpcPathParamsRule, "@azure-tools/typespec-azure-core");
});
    ...
    await tester
      .expect(
        `
          ...
          @pollingOperation(getStatus) @route("/lrRpcOp") op lrRpcOp is Azure.Core.LongRunningRpcOperation<{ @path foo: string }, {}, PollingStatus, StatusError>;
        `,
      )
      .toEmitDiagnostics([
        {
