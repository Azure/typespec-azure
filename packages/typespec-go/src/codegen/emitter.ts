...
export class Emitter {
  ...
  async emitApiViewPropertiesFile(containingModuleRelativePackagePath?: string): Promise<void> {
    const apiViewProperties = generateApiViewProperties(
      this.codeModel,
  ...
  private async recursiveEmit(
    ...
    const recursiveEmit = async (pkg: go.PackageContent, dir: string): Promise<void> => {
      await emitForPkg(pkg, async (name: string, content: string, subdir?: string) => {
        return await this.fs.write(
          `${dir}${subdir ? `${subdir}/` : ""}${this.filePrefix}${name}`,
          content,
        );
      });
...
function sortContent(pkg: go.PackageContent): void {
  ...
  for (const iface of pkg.interfaces) {
    ...
    iface.possibleTypes.sort((a: go.PolymorphicModel, b: go.PolymorphicModel) => {
      const aValue =
        a.discriminatorValue!.type.kind === "constant"
          ? (<go.ConstantValue>a.discriminatorValue!.literal).name
          : <string>a.discriminatorValue!.literal;
      const bValue =
        b.discriminatorValue!.type.kind === "constant"
          ? (<go.ConstantValue>b.discriminatorValue!.literal).name
          : <string>b.discriminatorValue!.literal;
  ...
  for (const respEnv of pkg.responseEnvelopes) {
    respEnv.headers.sort(
      (
        a: go.HeaderScalarResponse | go.HeaderMapResponse,
        b: go.HeaderScalarResponse | go.HeaderMapResponse,
      ) => {
  ...
  for (const client of pkg.clients) {
    if (client.instance?.kind === "constructable") {
      client.instance.constructors.sort((a: go.Constructor, b: go.Constructor) =>
        sortAscending(a.name, b.name),
      );
      if (client.instance.options.kind === "clientOptions") {
        client.instance.options.parameters.sort((a: go.ClientParameter, b: go.ClientParameter) =>
          sortAscending(a.name, b.name),
        );
      }
    }
    client.parameters.sort((a: go.ClientParameter, b: go.ClientParameter) =>
      sortAscending(a.name, b.name),
    );
