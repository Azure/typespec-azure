...
export function generateClientFactory(
  ...
  result += helpers.formatCommentAsBulletItem("credential", {
    summary: "used to authorize requests. Usually a credential from azidentity.",
  });
  result += helpers.formatCommentAsBulletItem("options", {
    summary: "pass nil to accept the default values.",
  });
  ...
  result += `func NewClientFactory(${clientFactoryParams
    ...
    .join(
      ", ",
    )}${clientFactoryParams.length > 0 ? "," : ""} options *arm.ClientOptions) (*ClientFactory, error) {\n`;
