 ...
 */
export function generateGoModFile(
  module: go.Module,
  options: go.Options,
  ...
  const match = existingGoMod.match(
    /github\.com\/Azure\/azure-sdk-for-go\/sdk\/azcore\s+v(\d+\.\d+\.\d+(?:-[a-zA-Z0-9_.-]+)?)/,
  );
  if (match) {
    if (match.length < 2) {
    ...
    if (semver.lt(existingVer, specifiedVer)) {
      ...
      existingGoMod = existingGoMod.replace(
        /github\.com\/Azure\/azure-sdk-for-go\/sdk\/azcore\s+v\d+\.\d+\.\d+(?:-[a-zA-Z0-9_.-]+)?/,
        azcore,
      );
    }
