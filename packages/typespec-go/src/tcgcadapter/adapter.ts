  ...
  private constructor(ctx: tcgc.SdkContext, options: GoEmitterOptions, emitterOutputDir: string) {
    ...
    if (this.options["containing-module"] && this.options.module) {
      throw new AdapterError(
        "InvalidArgument",
        "module and containing-module are mutually exclusive",
      );
    }
 ...
 */
function fixStutteringTypeNames(
  sdkPackage: tcgc.SdkPackage<tcgc.SdkHttpOperation>,
  packageName: string,
  ...
  const renameType = function (typeName: string): string {
    ...
    if (newName !== originalName && nameCollision(newName)) {
      collisions.push(
        `type ${originalName} was renamed to ${newName} which collides with an existing type name`,
      );
    }
    return newName;
  };
