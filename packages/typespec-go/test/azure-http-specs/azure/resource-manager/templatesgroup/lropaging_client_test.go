...
func TestNewLroPagingClient_BeginPostPagingLroWithBody(t *testing.T) {
	// Start the long-running operation with a request body
	poller, err := clientFactory.NewLroPagingClient().BeginPostPagingLroWithBody(ctx, resourceGroupExpected, "default", templatesgroup.VnetProfile{
		VnetID: to.Ptr("vnet1"),
	}, nil)
	require.NoError(t, err)
	...
	// Iterate through the first page
	require.True(t, pager.More(), "Expected at least one page of results")
	...
	// Iterate through the second page
	require.True(t, pager.More(), "Expected a second page of results")
	...
	// Verify no more pages
	require.False(t, pager.More(), "Expected no more pages after the second page")
}
