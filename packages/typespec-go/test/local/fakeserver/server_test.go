	...
	client, err := fakeserver.NewClientWithNoCredential("https://fake.endpoint", &fakeserver.ClientOptions{
		ClientOptions: policy.ClientOptions{
			Transport: fake.NewServerTransport(&fake.Server{
				GetInteger: func(_ context.Context, _ *fakeserver.ClientGetIntegerOptions) (resp azfake.Responder[fakeserver.ClientGetIntegerResponse], errResp azfake.ErrorResponder) {
					resp.SetResponse(http.StatusOK, fakeserver.ClientGetIntegerResponse{Value: to.Ptr[int64](0)}, nil)
					return
				},
			}),
		},
...
func TestCancelledContextPanic(t *testing.T) {
	...
	for range 500 {
		_, err := client.GetInteger(ctx, nil)
...
func TestGetIntegerFake(t *testing.T) {
	...
	client, err := fakeserver.NewClientWithNoCredential("https://fake.endpoint", &fakeserver.ClientOptions{
		ClientOptions: policy.ClientOptions{
			Transport: fake.NewServerTransport(&fake.Server{
				GetInteger: func(_ context.Context, _ *fakeserver.ClientGetIntegerOptions) (resp azfake.Responder[fakeserver.ClientGetIntegerResponse], errResp azfake.ErrorResponder) {
					resp.SetResponse(http.StatusOK, fakeserver.ClientGetIntegerResponse{
...
func TestDoubleDecodeQueryParam(t *testing.T) {
	...
	client, err := fakeserver.NewClientWithNoCredential("https://fake.endpoint", &fakeserver.ClientOptions{
		ClientOptions: policy.ClientOptions{
			Transport: fake.NewServerTransport(&fake.Server{
				DoubleDecode: func(_ context.Context, pathParam string, query string, _ *fakeserver.ClientDoubleDecodeOptions) (resp azfake.Responder[fakeserver.ClientDoubleDecodeResponse], errResp azfake.ErrorResponder) {
					receivedPath = pathParam
					receivedQuery = query
					resp.SetResponse(http.StatusNoContent, fakeserver.ClientDoubleDecodeResponse{}, nil)
					return
				},
			}),
		},
