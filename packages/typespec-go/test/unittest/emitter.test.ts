...
func main() {
  ...
  it("should recurse into the fake/ subdirectory", async () => {
    const generatedContent = `// Code generated by Microsoft (R) AutoRest Code Generator. DO NOT EDIT.
package fake`;

    const userContent = `package fake
// User written code`;

    await createTestFile("fake/generated.go", generatedContent);
    await createTestFile("fake/user.go", userContent);

    cleanupGeneratedFiles(testDir);

    // Verify only generated files were deleted in fake/
    expect(await fileExists("fake/generated.go")).toBe(false);
    expect(await fileExists("fake/user.go")).toBe(true);
  });

  it("should delete generated .go files with legacy do-not-edit comment on separate line", async () => {
    // Old autorest-generated example files have DO NOT EDIT on a separate line
    const legacyContent = `// Copyright (c) Microsoft Corporation. All rights reserved.
...
package main`;
    ...
    await createTestFile("legacy_example_test.go", legacyContent);

    cleanupGeneratedFiles(testDir);

    // Verify the file was deleted
    expect(await fileExists("legacy_example_test.go")).toBe(false);
  });
