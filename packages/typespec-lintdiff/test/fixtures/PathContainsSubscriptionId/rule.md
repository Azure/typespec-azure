---
validatorRuleId: PathContainsSubscriptionId
engine: spectral
tspLints: []
...
Resource group scoped CRUD paths must contain a `subscriptionId` parameter.
...
| -- | --------- | ----------- |
| `missing-subscription-id` | yes | Raw ARM-style PUT path omits the `subscriptionId` parameter |
