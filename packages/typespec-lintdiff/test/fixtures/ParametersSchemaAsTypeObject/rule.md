...
| unbased custom scalar without an effective emitted encoding format                                                                           | schema without `type`                                              | pass                 | `unbased-scalar-body`, `empty-encoded-scalar-body` |
| encoded scalar chain that resolves to a supported typed format, or whose unsupported nested format leaves an inherited primitive type intact | explicit primitive `type`                                          | violate              | `explicit-schema-type-bodies`                      |
...
| `unbased-scalar-body`         | false     | Unbased custom scalar emits an untyped schema                                                                                                                                                        |
...
| `explicit-schema-type-bodies` | true      | Scalar, direct, nested, or inline-property encoding (including supported and rejected nested formats), file, enum, member, variant, tuple, literal, and string-template bodies emit non-object types |
