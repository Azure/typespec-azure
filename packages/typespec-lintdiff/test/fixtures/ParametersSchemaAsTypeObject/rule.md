...
- an encoded model-property body backed by an inline schema, including an unnamed template
  instance, should violate because the encoding replaces the inline schema type; `@friendlyName`
  makes the template instance referenced, so validator resolution follows its object definition
...
| encoded model property backed by an inline schema, including an unnamed template instance                                                     | `getSchemaOrRef(ModelProperty)` → `resolveProperty` → inline `getSchemaOrRef` → `applyEncoding`             | encoding replaces the inline schema `type`                        | violate              | `explicit-schema-type-bodies`                      |
...
| `encoded-model-property-body` | false     | Validator resolution hides the encoded type beside scalar, union, and friendly-named template `$ref` schemas                                                                                                           |
| `multipart-body`              | false     | Multipart parts emit without a body schema                                                                                                                                                                             |
| `explicit-schema-type-bodies` | true      | Scalar, direct, nested, or inline-property encoding (including template instances and supported or unsupported formats), file, enum, member, variant, tuple, literal, and string-template bodies emit non-object types |
