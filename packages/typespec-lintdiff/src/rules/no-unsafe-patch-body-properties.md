...
This source rule is ARM-only. Lintdiff's mixed ARM/data-plane `all` ruleset
enables it in place of the three legacy implementations and uses an upward
provider-namespace lookup for isolation. Official ARM promotion removes only
that isolation guard; the proposed official rule remains **disabled** in the
shared resource-manager ruleset. No official default enablement changes here.
