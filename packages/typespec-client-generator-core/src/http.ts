...
export function getSdkHttpParameter(
  ...
  if (isPathParam(context.program, param) || location === "path") {
    ...
    return diagnostics.wrap({
      ...
      kind: "path",
      urlEncode,
      explode: (httpParam as HttpOperationPathParameter)?.explode,
      style: (httpParam as HttpOperationPathParameter)?.style,
      allowReserved: (httpParam as HttpOperationPathParameter)?.allowReserved,
