...
export function getSdkUnionWithDiagnostics(
  ...
  let retval: SdkType | undefined = context.getReferencedTypeCache().get(type);
  const diagnostics = createDiagnosticCollector();
...
export function getSdkModelWithDiagnostics(
  ...
  const diagnostics = createDiagnosticCollector();
  let sdkType = context.getReferencedTypeCache().get(type) as SdkModelType | undefined;
  ...
  if (!sdkType) {
    ...
    if (type.baseModel) {
      sdkType.baseModel = context.getReferencedTypeCache().get(type.baseModel) as
        | SdkModelType
        | undefined;
...
function getSdkEnumWithDiagnostics(
  ...
  const diagnostics = createDiagnosticCollector();
  let sdkType = context.getReferencedTypeCache().get(type) as SdkEnumType | undefined;
  if (!sdkType) {
    sdkType = {
...
function updateReferencedTypeMap(context: TCGCContext, type: Type, sdkType: SdkType) {
  ...
  context.getReferencedTypeCache().set(type, sdkType);
}
...
function updateUsageOverride(context: TCGCContext): [void, readonly Diagnostic[]] {
  const diagnostics = createDiagnosticCollector();
  for (const sdkType of context.getReferencedTypeCache().values()) {
...
function updateSpreadModelUsageAndAccess(context: TCGCContext): void {
  for (const [_, sdkType] of context.getReferencedTypeCache().entries()) {
    if (
      sdkType.kind === "model" &&
