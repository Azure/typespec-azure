...
it("enum discriminator model", async () => {
  ...
  const kind = golden.properties.find((x) => x.kind === "property" && x.serializedName === "kind");
  ok(kind);
...
it("additionalProperties of same type", async () => {
  ...
  const NonAdditionalPropertiesModel = models.find((x) => x.name === "NoAdditionalPropertiesModel");
  ok(
