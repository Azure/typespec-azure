import { ModelProperty, Type, StringLiteral } from "@typespec/compiler";
import {
  BasicTestRunner,
...
describe("my library", () => {
  ...
  const browseString = createbrowseTestDecorator(`"helloThisISArgQuery"`);
    ...
    it("test @about", async () => {
      ...
      const keywordValues = keywords.type.values.filter((value: Type) => value.kind === "String").map((value: Type) => (<StringLiteral>value).value);
      deepEqual(keywordValues, ['a', 'c', 'b']);

      strictEqual(learnMoreDocs.kind, "ModelProperty");
      strictEqual(learnMoreDocs.type.kind, "Tuple");
      const learnMoreDocsValues = learnMoreDocs.type.values.filter((value: Type) => value.kind === "String").map((value: Type) => (<StringLiteral>value).value);
  ...
  export function createAboutTestDecorator(displayName?: string, keywords?: string, learnMoreDocs?: string) {
