...
> model. If your existing operation-status payload has extra top-level members (for example
> `operations` or `resourceId`) or different wire semantics, keep or author an explicit status model
