...
Both `ConversationProperties` and `ResponseProperties` include a required `input: InputItem`
property. `InputItem.type` selects whether the input is a message, function call, function-call
output, or compaction summary. Clients provide the fields for that variant, but `id`, `role`, and
`status` are service-assigned and read-only.
