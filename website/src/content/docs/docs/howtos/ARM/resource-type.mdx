...
Tenant resources use the `TrackedResource<TProperties>` as their base resource type, where
`TProperties` is the properties model for the RP-specific properties of the resource. Here is an
example:
...
resource-group boundaries and require additional security review. A PUT operation generated for a
tenant resource triggers the `no-tenant-level-apis` lint rule. Prefer subscription or resource-group
...
- **Availability Zones**: Identifying the availability zones where a resource is deployed.
- **Billing Data**: A standard mechanism for specifying billing properties for prepaid resources.

### Availability Zones

Spread `AvailabilityZonesProperty` into the resource model to add the standard optional `zones`
envelope property:
