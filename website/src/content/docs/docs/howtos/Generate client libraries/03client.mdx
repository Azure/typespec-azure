...
By default, each namespace with the `@service` decorator is generated as a root client. The client name is the namespace name with `Client` appended as a suffix.
...
### The client has operation groups with a single layer of child groups
...
### The client has multi-layer child operation groups
