...
2. **Auto-Merge with `autoMergeService: true`**: Set `autoMergeService: true` on the `@client` decorator to automatically merge all operations and sub clients from every listed service into the single client. Without this flag, the client will have no operations — you would need to define child `@client` decorators or use `is` to map operations explicitly.

3. **Version Dependencies**: If you want to specify each service's API version used in the single client, you could use `@useDependency` to declare which version of each service the client supports. Otherwise, the latest version of each service will be used by default.
...
# generated operations/_operations.py
class AIOperations:
    def __init__(self, client, api_version: str = "av1") -> None: ...
...
client.ai.a_test()  # uses api-version av1
...
// AI operations use ServiceA's api-version (av1)
...
public final class AIClient {
    // Uses ServiceA's api-version (av1)
    public void aTest();
}
...
aiClient.aTest();  // uses api-version av1
...
aiClient.ATest(context.Background(), &AIClientATestOptions{})  // uses api-version av1
