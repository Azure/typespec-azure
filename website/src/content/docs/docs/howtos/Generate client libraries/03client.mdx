...
TypeSpec allows you to define a single client that combines operations from multiple services. This is useful when you want to provide a unified client experience for related services that share a common endpoint, but require different versioning.

#### Key Considerations for Emitters

When implementing support for multi-service clients, emitters should keep the following in mind:

1. **Service Array**: The `@client` decorator's `service` property accepts an array of services (e.g., `service: [ServiceA, ServiceB]`).

2. **Version Dependencies**: When combining versioned services, you must use `@useDependency` to declare which version of each service the client supports. This is required for multi-service clients.

3. **API Version Handling**:
   - Each child operation group maintains its own `apiVersions` array based on its originating service.
   - Each operation group has its own `apiVersion` client parameter with the appropriate default value from `@useDependency`.

4. **Operation Group Structure**: Operations from each service are organized into separate operation groups (based on interfaces or namespaces from the original services).

5. **Client Initialization Parameters**:
   - The root client has a shared `endpoint` parameter.
   - Each child operation group has its own `apiVersion` parameter typed to its service's version enum.

6. **Restrictions**:
   - Only one explicit client can be defined when using multiple services.
   - `@clientLocation` cannot be used to create new operation groups (only move to existing ones) when the client has multiple services.
