...
It can be used to move operations to an existing sub client defined by `Interface` or `Namespace`, or move to the root client of the namespace with the `@service` decorator.
...
### Language-specific decorator options

Most TCGC decorators whose final parameter is `scope` accept either the legacy string form, such as
`"csharp"`, or the typed `DecoratorOptions` value-bag form, such as `#{ scope: "csharp" }`.
Existing string scopes remain supported and are not deprecated. Scope expressions continue to
support comma-separated emitters and negation.

The `@client` and `@clientInitialization` decorators already have options bags, so set `scope`
directly in `ClientOptions` or `ClientInitializationOptions` rather than passing a separate value
bag. Their legacy positional scope argument is also still supported. If both locations specify
different effective scopes, TCGC reports `conflicting-scope` and uses the scope in the options bag.
Equivalent expressions, including reordered or regrouped expressions, do not conflict.

All options are resolved through model inheritance. For example, a model derived from
`ClientOptions` can inherit `scope` and `service` from a base model while declaring `name` itself.
Likewise, a model derived from `ClientInitializationOptions` can inherit `scope`, `parameters`, or
`initializedBy`.

For backward compatibility, a raw client-initialization parameters model that happens to contain a
property named `scope` is still treated as parameters unless TCGC recognizes it as
`ClientInitializationOptions`, including through inheritance or a mapped source model. For a raw
parameters model, `scope` is therefore generated as a client parameter rather than interpreted as
an emitter selector.
...
7. **Operation Names**: Operations combined into the same generated client must have distinct client names for most languages. C# supports same-named operations with distinct signatures as overloads, so TCGC reports `duplicate-client-name-warning` instead of an error for C# operation-name collisions. Verify that the generated signatures form valid overloads and suppress the warning only when the collision is intentional; otherwise, use `@clientName` to resolve it.
...
:::warning[Diagnostic: `inconsistent-multiple-service-dependency`]
When services merged into one client depend on different versions of a shared library (via `@useDependency`), TCGC emits a warning:

> Services merged into client "\{clientName\}" depend on different versions of "\{dependencyName\}": \{versions\}.

For example, if `ServiceA` depends on `SharedLib.v1` and `ServiceB` depends on `SharedLib.v2`, and both are merged into a single client, this diagnostic fires. Diverging dependency versions can cause TCGC to produce duplicated or diverged models in the generated SDK. To resolve this, align the services to use the same version of the shared dependency.
:::
