...
The `@paramAlias` decorator lets you use a different name for a parameter in client initialization than its name in the operation.
This is useful when elevating operation parameters to client level via `@clientInitialization` and you want a more descriptive client-level name.
...
@route("/files/{blob}")
@get
op download(@path blob: string): bytes;
```
...
@@clientInitialization(MyService,
  {
    parameters: MyCustomizations.MyClientInitialization,
  }
);
```

```python
# The 'blob' operation parameter is elevated to client init as 'blob_name'
class MyServiceClient:
    def __init__(self, endpoint: str, blob_name: str, **kwargs: Any) -> None:
        ...

    def download(self, **kwargs: Any) -> Iterator[bytes]:
        ...
```
...
{
    // The 'blob' operation parameter is elevated to client init as 'blobName'
    public MyServiceClient(Uri endpoint, string blobName);

    public virtual Response Download(CancellationToken cancellationToken = default);
}
```
...
export class MyServiceClient {
  // The 'blob' operation parameter is elevated to client init as 'blobName'
  constructor(endpointParam: string, blobName: string, options?: MyServiceClientOptionalParams);

  download(options?: DownloadOptionalParams): Promise<DownloadResponse>;
}
```

```java
public final class MyServiceClientBuilder {
    // The 'blob' operation parameter is elevated to client init as 'blobName'
    public MyServiceClientBuilder blobName(String blobName);
    public MyServiceClient buildClient();
}

public final class MyServiceClient {
...
```go
// The 'blob' operation parameter is elevated to client init as 'blobName'
func NewMyServiceClientWithNoCredential(
    endpoint string,
    blobName string,
    options *MyServiceClientOptions,
) (*MyServiceClient, error)

func (client *MyServiceClient) Download(
    ctx context.Context,
    options *MyServiceClientDownloadOptions,
) (MyServiceClientDownloadResponse, error)
```
