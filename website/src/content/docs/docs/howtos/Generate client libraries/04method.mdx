...
## Using `@scope` to limit operations to specific languages

The `@scope` decorator allows you to include or exclude operations and model properties from specific language emitters. By default, all elements are emitted for all languages.

You can use `@scope` to:

- Target specific languages: `@scope("python, java")` — only emit for Python and Java
- Exclude specific languages: `@scope("!csharp")` — emit for all except C#
- Exclude multiple languages: `@scope("!(java, python)")` — emit for all except Java and Python
...
// Only generate the `create` operation for Python and Java
@@scope(PetStore.create, "python, java");
```

```python
class PetStoreClient:
    # The `create` operation IS generated for Python
    def list(self, **kwargs: Any) -> List[Pet]: ...
    def create(self, pet: Pet, **kwargs: Any) -> Pet: ...
    def read(self, name: str, **kwargs: Any) -> Pet: ...
```

```csharp
// The `Create` operation is NOT generated for C#
public partial class PetStoreClient
{
    public virtual Response<IReadOnlyList<Pet>> GetAll(CancellationToken cancellationToken = default);
    // No Create method — excluded by @scope
    public virtual Response<Pet> Read(string name, CancellationToken cancellationToken = default);
}
```
...
public final class PetStoreClient {
    // The `create` operation IS generated for Java
...
```go
// The `Create` operation is NOT generated for Go
func NewPetStoreClientWithNoCredential(endpoint string, options *PetStoreClientOptions) (*PetStoreClient, error)
func (client *PetStoreClient) List(ctx context.Context, options *PetStoreClientListOptions) (PetStoreClientListResponse, error)
// No Create method — excluded by @scope
func (client *PetStoreClient) Read(ctx context.Context, name string, options *PetStoreClientReadOptions) (PetStoreClientReadResponse, error)
```

</ClientTabs>

The `@scope` decorator can also be applied to model properties to include or exclude them from specific languages:
...
The `@responseAsBool` decorator indicates that a HEAD operation should return a boolean value instead of the raw HTTP response. When applied:

- A `2xx` response returns `true`
- A `404` response returns `false` (instead of raising an error)
- All other status codes still raise errors
...
@@responseAsBool(PetStore.exists);
```

```python
class PetStoreClient:
    # Returns bool instead of None — True if pet exists, False if 404
...
public partial class PetStoreClient
{
    // Returns bool — true if pet exists, false if 404
...
```java
// TODO: fill in Java example manually — @responseAsBool convenience not fully reflected in Java output
```

```go
// NOT_SUPPORTED — Go emitter does not yet support @responseAsBool
```
