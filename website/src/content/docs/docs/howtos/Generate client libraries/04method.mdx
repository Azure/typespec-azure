...
## Return a boolean from HEAD operations

The `@responseAsBool` decorator transforms a HEAD operation to return a `boolean` instead of `void`.
This is commonly used for existence-check operations where 2xx responses map to `true` and 404 responses map to `false`.
...
```typescript
export type ExistsResponse = { body: boolean };
...
```java
// Java does not yet generate a boolean convenience method for @responseAsBool
public Response<BinaryData> existsWithResponse(String name, RequestOptions requestOptions);
```
...
## Filtering operations by language scope

The `@scope` decorator controls whether an operation is included in generated client SDKs for specific languages.
This is useful when certain operations should only appear in specific language SDKs.
...
```python
# Python includes both operations (createPet is scoped to python)
class PetStoreClient:
    def list_pets(self, **kwargs: Any) -> list[Pet]:
        ...

    def create_pet(self, pet: Union[Pet, JSON, IO[bytes]], **kwargs: Any) -> Pet:
        ...
```

```csharp
// C# only includes listPets (createPet is excluded by @scope("python"))
public partial class PetStoreClient
{
    public virtual Response<IReadOnlyList<Pet>> ListPets(CancellationToken cancellationToken = default);
}
```

```typescript
// TypeScript includes both operations (scope filtering may vary by emitter)
export class PetStoreClient {
  listPets(options?: ListPetsOptionalParams): Promise<Pet[]>;
  createPet(pet: Pet, options?: CreatePetOptionalParams): Promise<Pet>;
}
```

```java
public final class PetStoreClient {
    // Java only includes listPets (createPet is excluded by @scope("python"))
    public List<Pet> listPets();
}
```

```go
// Go only includes ListPets (CreatePet is excluded by @scope("python"))
func (client *PetStoreClient) ListPets(
    ctx context.Context,
    options *PetStoreClientListPetsOptions,
) (PetStoreClientListPetsResponse, error)
```
