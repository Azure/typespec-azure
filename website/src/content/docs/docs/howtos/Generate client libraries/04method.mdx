...
Replace or remove a parameter in an operation:
...
Replace the return type of an operation:
...
@@override(getData, replaceResponse(getData, RichResponse));
```
...
def get_data(self) -> RichResponse:
    ...
```
...
public virtual Response<RichResponse> GetData(CancellationToken cancellationToken = default)
```
...
export async function getData(): Promise<RichResponse>;
```
...
model EnrichedResponse {
  results: string[];
  totalCount: int32;
}

// Chain: make query required -> make limit required -> change response
#suppress "experimental-feature" "using replaceParameter"
...
#suppress "experimental-feature" "using replaceResponse"
@@override(search, replaceResponse(step2, EnrichedResponse));
```
...
# Both query and limit are now required, response is EnrichedResponse
def search(self, query: str, limit: int) -> EnrichedResponse:
    ...
```
...
```csharp
public partial class EnrichedResponse
{
    public IList<string> Results { get; }
    public int TotalCount { get; }
}

// Both parameters required, returns EnrichedResponse
public virtual Response<EnrichedResponse> Search(string query, int limit, CancellationToken cancellationToken = default)
```
...
// Both parameters required, returns EnrichedResponse
export async function search(query: string, limit: number): Promise<EnrichedResponse>;
```
...
```java
public final class EnrichedResponse {
    public List<String> getResults();
    public int getTotalCount();
}

// Both parameters required, returns EnrichedResponse
public EnrichedResponse search(String query, int limit);
```
