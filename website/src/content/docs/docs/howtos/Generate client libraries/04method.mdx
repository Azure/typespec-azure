...
An intentional `void` or `bytes` replacement reports the `override-response-replacement` warning; suppress it at the customization site after confirming the replacement is expected. An override operation with any other response incompatible with the original reports the `override-response-mismatch` error. Use these functions instead of declaring an arbitrary incompatible return type.
