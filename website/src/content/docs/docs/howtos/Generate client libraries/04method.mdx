...
Replace only the generated client method response with `void` while preserving the original HTTP
responses, including their status codes, response bodies, and error metadata:
...
Replace only the generated client method response with the raw HTTP response bytes while preserving
the original HTTP response metadata:
...
Both response transformations can be chained with parameter transformations and scoped through
`@override`. Replacing a pageable operation's response with either `void` or `bytes` also makes the
generated method non-pageable.

An intentional `void` or `bytes` replacement reports the `override-response-replacement` warning;
suppress it at the customization site after confirming the replacement is expected. An override
operation with any other response incompatible with the original reports the
`override-response-mismatch` error. Use these functions instead of declaring an arbitrary
