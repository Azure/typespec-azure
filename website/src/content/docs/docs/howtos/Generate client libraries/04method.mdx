...
- Replace the generated response with `void` or raw bytes
...
When applied, the method parameters come from the override operation while the underlying HTTP operation remains unchanged. TCGC maps method parameters to operation parameters based on name matching. The override operation's declared return type is ignored unless the operation was produced by `replaceResponseWithVoid` or `replaceResponseWithBytes`.
...
Replace only the generated client method response with `void` while preserving the original HTTP responses, including their status codes, response bodies, and error metadata:
...
#suppress "@azure-tools/typespec-client-generator-core/override-response-replacement" "The client intentionally ignores the response body."
@@override(MyService.create, replaceResponseWithVoid(MyService.create));
```
...
```go
type MyServiceClientCreateResponse struct {}

func (client *MyServiceClient) Create(ctx context.Context, widget Widget, options *MyServiceClientCreateOptions) (MyServiceClientCreateResponse, error)
```
...
Replace only the generated client method response with the raw HTTP response bytes while preserving the original HTTP response metadata:
...
#suppress "@azure-tools/typespec-client-generator-core/override-response-replacement" "The client intentionally exposes the raw response bytes."
@@override(MyService.download, replaceResponseWithBytes(MyService.download));
```
...
```typescript
export type DownloadResponse = { body: Uint8Array };

export async function download(
  context: Client,
  options?: DownloadOptionalParams,
...
Both response transformations can be chained with parameter transformations and scoped through `@override`. Replacing a pageable operation's response with either `void` or `bytes` also makes the generated method non-pageable.

An intentional `void` or `bytes` replacement reports the `override-response-replacement` warning; suppress it at the customization site after confirming the replacement is expected. Declaring a return type directly on a plain override operation does not replace the generated method response or produce a response diagnostic. Use these transformation functions when you intend to replace the response.
