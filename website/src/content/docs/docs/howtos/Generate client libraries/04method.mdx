...
// OutputModel - This is the input I need
type OutputModel struct {
    // Name - Id of this object
    Name string `json:"name"`
}

// PetStoreNamespaceClient is the client for interacting with Output resources.
type PetStoreNamespaceClient struct {
    endpoint string
    pl       runtime.Pipeline
}

// NewClient creates a new instance of the PetStoreNamespaceClient.
func NewClient(endpoint string, cred azcore.TokenCredential, options *ClientOptions) (*PetStoreNamespaceClient, error) {
    pl := runtime.NewPipeline("petstorenamespace", "v0.1.0", runtime.PipelineOptions{}, &options.ClientOptions)
...
// GetModel - Read my resource
func (c *PetStoreNamespaceClient) GetModel(ctx context.Context, name string) (OutputModel, error) {
    req, err := runtime.NewRequest(ctx, http.MethodGet, c.endpoint+"/outputs/"+name)
    if err != nil {
    ...
    if resp.StatusCode != http.StatusOK {
        return OutputModel{}, runtime.NewResponseError(resp)
    }

    var result OutputModel
    err = runtime.UnmarshalAsJSON(resp, &result)
    return result, err
}
...
func (client *PetStoreNamespaceClient) GetModel(ctx context.Context, name string) (OutputModel, error)
...
```go
func (c *petStoreNamespaceClient) GetModelWithResponse(ctx context.Context, name string, options *RequestOptions) (Response[OutputModel], error) {
    // Low-level call returning Response with HTTP details
}
...
```go
// Client is the Analyze service client.
type Client struct {
	BaseURL    string
	HTTPClient *http.Client
}

// NewClient creates a new Analyze client.
...
// AnalyzeResult is the response from the analyze operation.
type AnalyzeResult struct {
	// TODO: define actual response fields
	Result string `json:"result,omitempty"`
}

// CompletionInput is the input model for completions operation.
// Input supports either string or []string.
type CompletionInput struct {
	Input interface{} `json:"input"` // string or []string
}

// CompletionResult is the response from the completions operation.
type CompletionResult struct {
	// TODO: define actual response fields
	Output string `json:"output,omitempty"`
}
func (c *Client) Analyze(ctx context.Context, mode string, image []byte) (*AnalyzeResult, error) 

func (c *Client) Completions(ctx context.Context, input CompletionInput) (*CompletionResult, error) 
...
// Client for Foo operations.
type Client struct {
    BaseURL    string
    HTTPClient *http.Client
}

// NewClient creates a new Foo client.
func NewClient(baseURL string, httpClient *http.Client) *Client {
    if httpClient == nil {
        httpClient = http.DefaultClient
    }
...
func (c *Client) A(ctx context.Context, x int32) (float64, error)

func (c *Client) B(ctx context.Context, x string) (int64, error) 
```
