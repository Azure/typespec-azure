...
```java
// TODO: fill in Java example manually — Java emitter does not yet generate boolean return type for @responseAsBool
```

```go
// TODO: fill in Go example manually — Go emitter does not yet support @responseAsBool
```
...
- All successful (2xx) responses are modeled as returning `true`
- A 404 response is modeled as returning `false` (moved from exceptions to normal responses)
