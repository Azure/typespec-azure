...
export function listItems(options?: ListItemsOptionalParams): PagedAsyncIterableIterator<Item>;
```
...
```typescript
// When @disablePageable is applied, the operation returns the raw response model
export function listItems(options?: ListItemsOptionalParams): Promise<ItemListResult>;
```
