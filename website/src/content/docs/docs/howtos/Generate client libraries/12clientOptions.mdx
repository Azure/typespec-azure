...
| `value`   | `unknown`                  | The option value. Primitive, array, object, nested, and model-reference values are supported; the consuming emitter determines the expected shape. |
...
Model references are converted to TCGC SDK types for the scoped emitter. Scoped customizations on the referenced model, such as `@alternateType`, are applied before the emitter receives the option value.
...
The function returns the option value as `unknown`, or `undefined` when the named option isn't present. Model-reference values are returned as their converted TCGC SDK type rather than as raw TypeSpec models. Narrow or validate the value before using it, then call it once for each option key your emitter supports.
