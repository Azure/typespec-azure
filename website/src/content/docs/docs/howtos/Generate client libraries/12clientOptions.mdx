...
The function returns the option value as `unknown`, or `undefined` when the named option isn't present. Call it once for each option key your emitter supports.
