...
## Overriding Client API Versions with `@clientApiVersions`

The `@clientApiVersions` decorator allows you to explicitly specify which API versions a client should support, overriding the versions inferred from the `@versioned` decorator. This is useful when:

- You want to add additional preview versions that clients should be able to target
- You need to restrict which versions the client exposes compared to the service's full version history
- You're building a client that should support versions not yet reflected in the spec's `@versioned` enum
...
@route("/pets")
@get
op list(@query("api-version") apiVersion: string): void;
```
...
// Override the client's API version list with a custom enum
enum ClientVersions {
  v1: "v1",
  v2: "v2",
  v3: "v3",
...
@@clientApiVersions(PetStore, ClientVersions);
```

The `@clientApiVersions` decorator primarily affects the TCGC type graph used by emitter developers. The client's API version parameter will reflect the versions from the provided enum, and emitters can use this information to generate appropriate version handling code.
