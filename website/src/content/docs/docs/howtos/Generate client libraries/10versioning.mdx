...
## Adding Additional API Versions with `@clientApiVersions`

The `@clientApiVersions` decorator allows you to specify additional API versions that the client can support beyond what is defined by the service's `@versioned` decorator. This is useful when you want to expose a complete API version enum in the generated client without requiring the entire specification to be annotated with versioning decorators.
...
op get(@query("api-version") apiVersion: Versions): void;
```
...
// Define a complete API version enum that includes older versions
...
# The generated ServiceVersion enum now includes all versions: v1, v2, v3, v4, v5
# Default api_version is "v5" (latest)
client = MyServiceClient(endpoint=..., credential=...)

# You can also target an older version
client = MyServiceClient(endpoint=..., credential=..., api_version="v1")
```

```csharp
// ServiceVersion enum now includes all versions
public enum ServiceVersion
{
    V1 = 1,
    V2 = 2,
...
// Default uses latest version (v5)
var client = new ServiceClient(new Uri("https://my-service.azure.com"));

// Target a specific version
var options = new ServiceClientOptions(ServiceVersion.V1);
var client = new ServiceClient(new Uri("https://my-service.azure.com"), options);
```
...
// Default uses latest version (v5)
ServiceClient client = new ServiceClientBuilder().buildClient();
```

```go
// Go does not export enum types for API versions, so no changes are needed.
```
