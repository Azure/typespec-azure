...
### Add api version validation for elements

The `@added` decorator in TypeSpec serves as a versioning validation that indicates when a specific API element was first introduced. When applied to models, operations, or parameters, `@added` marks the version at which these elements became available.
...
**Note**: current python emitter only add validation for operation and parameters and don't handle versioning for models.
...
@added(Versions.v2023_11_02)
@route("/stable")
op stableFunctionality(
  @body stableModel: StableModel,
): void;

@route("/new_stable")
op paramFunctionality(
  @added(Versions.v2023_11_02) @query param?: string
): void;
```
...
```python
@api_version_validation(
    params_added_on={"2023-11-02": ["param"]},
    api_versions_list=["2023-11-01-preview", "2023-11-02"],
)
def param_functionality(
    self, *, param: Optional[str] = None, **kwargs: Any
) -> None:
...
```
