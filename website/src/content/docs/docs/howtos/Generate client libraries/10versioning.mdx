...
### Body Model Properties Named `apiVersion`

API version detection by name only applies to HTTP metadata parameters (header, query, path, or cookie parameters). A body model property named `apiVersion` or `api-version` will **not** be treated as an API version parameter.
...
model MyRequestBody {
  // This is a regular body property, NOT treated as an API version parameter
  apiVersion: string;

  data: string;
}

op createResource(@body body: MyRequestBody): void;
```

In this example, `apiVersion` inside the body model is a normal property and will not be elevated to the client or given any special API version behavior.
