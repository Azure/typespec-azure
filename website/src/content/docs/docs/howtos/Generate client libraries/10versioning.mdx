...
client = MyServiceClient(endpoint=..., credential=...)
client.list_resources()  # Uses client-level api-version (default "2024-04-01")
client.get_snapshot(api_version="2023-11-01")  # Operation-level parameter
```

```csharp
public virtual Response ListResources(CancellationToken cancellationToken = default);
...
export function listResources(
  context: Client,
  options?: ListResourcesOptionalParams,
): Promise<void>;
...
```go
func (client *ServiceClient) ListResources(ctx context.Context, options *ListResourcesOptions) (ListResourcesResponse, error)
func (client *ServiceClient) GetSnapshot(ctx context.Context, apiVersion string, options *GetSnapshotOptions) (GetSnapshotResponse, error)
```
