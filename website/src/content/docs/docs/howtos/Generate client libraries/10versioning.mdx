...
```typescript
// update typespec operation to `op stableFunctionality(@header apiVersion: Versions, @body stableModel: StableModel): void;`
...
export function _stableFunctionalitySend(
  context: Client,
  ...
  return context.path("/").post({
    ...
    body: stableModelSerializer(stableModel),
  });
}

export async function _stableFunctionalityDeserialize(
...
export async function stableFunctionality(
  ...
  const result = await _stableFunctionalitySend(context, stableModel, options);
  return _stableFunctionalityDeserialize(result);
}
```
...
```typescript
// update typespec operation to
// @route("/preview")
// op previewFunctionality(@header apiVersion: Versions, @body previewModel: PreviewModel): void;
// @route("/stable")
// op stableFunctionality(@header apiVersion: Versions, @body stableModel: StableModel): void;`
export interface StableModel {
  stableFeature: string;
}
...
export type Versions = "2023-11-02";
...
export function _stableFunctionalitySend(
  ...
  return context.path("/stable").post({
    ...operationOptionsToRequestParameters(options),
    contentType: "application/json",
    body: stableModelSerializer(stableModel),
  });
...
export function _previewFunctionalitySend(
  context: Client,
  previewModel: PreviewModel,
  options: PreviewFunctionalityOptionalParams = { requestOptions: {} },
): StreamableMethod {
  return context.path("/preview").post({
    ...operationOptionsToRequestParameters(options),
    contentType: "application/json",
    body: previewModelSerializer(previewModel),
  });
}

export async function _previewFunctionalityDeserialize(
  result: PathUncheckedResponse,
): Promise<void> {
  const expectedStatuses = ["204"];
  if (!expectedStatuses.includes(result.status)) {
...
export async function previewFunctionality(
  ...
  const result = await _previewFunctionalitySend(context, previewModel, options);
  return _previewFunctionalityDeserialize(result);
}
```
...
```typescript
// Not found decorator @isApiVersion in @azure-tools/typespec-client-generator-core.
```
