...
The configuration flag `api-version` allows you to toggle the behavior that our emitters will generate. For single-service packages, it accepts a string value. For multi-service packages, it can also be an object that maps service namespaces to version strings. Represent each segment of a nested namespace as a nested object (see [Per-service versioning](#per-service-versioning-multi-service-packages)).
...
With the configuration above, the generated client uses version `2024-01-01` for `Microsoft.Network` operations and `2024-05-01` for `Microsoft.Compute` operations. TCGC also uses each resolved service version when selecting versioned example files.

If `Compute` is omitted, `Microsoft.Network` uses `2024-01-01` while `Microsoft.Compute` defaults to its latest version, `2024-09-01`.
