...
@clientDoc("Base class for all shape objects that can be drawn on a canvas", DocumentationMode.replace)
model Shape {
  @doc("The coordinates of the shape")
  @clientDoc("The x,y coordinates where the shape will be positioned on the canvas", DocumentationMode.replace)
  position: Point;
}
...
The `@clientDoc` decorator can be used on:
- Models
- Model properties
- Operations
