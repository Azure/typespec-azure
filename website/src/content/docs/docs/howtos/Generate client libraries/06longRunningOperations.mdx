...
:::caution
The `@markAsLro` decorator is legacy functionality from `Azure.ClientGenerator.Core.Legacy`. It should **only** be used when standard TypeSpec LRO patterns are not sufficient and you have explicit permission from SDK architects.
:::

### `@markAsLro`

Forces an operation to be treated as a long-running operation by SDK generators, even if it doesn't follow standard LRO patterns. When applied, the generated client will produce poller/LRO method signatures for the operation.
...
@markAsLro
@route("/long-operation")
@post
op startLongOperation(): void;
```
