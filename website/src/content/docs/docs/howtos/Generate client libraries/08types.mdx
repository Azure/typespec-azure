...
## Deserialize empty strings as null

The `@deserializeEmptyStringAsNull` decorator specifies that a string-typed model property should deserialize empty strings (`""`) as `null`.
This is useful when APIs return empty strings but the client should treat them as absent values.

:::caution
This decorator only applies to properties of type `string` or scalar types derived from `string`.
:::
...
class Resource:
    name: str = rest_field()
    # Empty string "" from the API will be deserialized as None
...
```csharp
// The emitter uses the decorator metadata to convert empty strings to null during deserialization
public partial class Resource
{
    public string Name { get; }
    public string Description { get; }  // "" deserialized as null
}
```
...
export interface Resource {
  name: string;
  description: string; // Empty string deserialized as null at runtime
}
```
...
public final class Resource {
    public String getName();
    public String getDescription();  // "" deserialized as null
}
```
