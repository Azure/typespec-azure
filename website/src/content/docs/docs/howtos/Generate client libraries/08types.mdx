...
The `@deserializeEmptyStringAsNull` decorator indicates that a model property of type `string` (or a scalar derived from `string`) should be deserialized as `null` when its value is an empty string (`""`). This is useful when a service sends empty strings to represent missing or unset values.
...
When the service returns `{ "nickname": "" }`, the generated client will deserialize the `nickname` property as `null` instead of an empty string.

:::note
The `@deserializeEmptyStringAsNull` decorator only affects deserialization behavior at runtime — it does not change the model declaration in generated code. The property type remains `string` in all languages, but the deserialization logic will convert empty strings to null values.
:::
