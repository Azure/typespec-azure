...
## Deserializing Empty Strings as Null (`@deserializeEmptyStringAsNull`)

For model properties of type `string` (or a scalar type derived from `string`), you can use `@deserializeEmptyStringAsNull` to indicate that an empty string value (`""`) should be deserialized as `null` in the generated client.

This is useful when a service returns empty strings to represent missing or unset values.
...
The model declaration is the same. During deserialization, if the service returns `""` for `description` or `website_url`, the SDK deserializes it as `None`.

```python
class Widget:
    name: str
    description: Optional[str]  # "" from service → None
    website_url: Optional[str]  # "" from service → None
```

</ClientTabItem>
<ClientTabItem lang="csharp">

The model declaration is the same. During deserialization, if the service returns `""` for `Description` or `WebsiteUrl`, the SDK deserializes it as `null`.

```csharp
public partial class Widget
{
    public string Name { get; }
    public string Description { get; }  // "" from service → null
    public Uri WebsiteUrl { get; }      // "" from service → null
}
```
...
The model declaration is the same. During deserialization, if the service returns `""` for `description` or `websiteUrl`, the SDK deserializes it as `undefined`.

```typescript
export interface Widget {
  name: string;
  description?: string; // "" from service → undefined
  websiteUrl?: string; // "" from service → undefined
}
```
...
public final class Widget {
    public String getName();
    public String getDescription();  // "" from service → null
    public String getWebsiteUrl();   // "" from service → null
}
```
...
The model declaration is the same. During deserialization, if the service returns `""` for `Description` or `WebsiteURL`, the SDK deserializes it as `nil`.

```go
type Widget struct {
    Name        *string
    Description *string  // "" from service → nil
    WebsiteURL  *string  // "" from service → nil
}
```
...
:::note
This decorator can only be applied to model properties whose type is `string` or a scalar type derived from `string` (e.g., `url`). Applying it to other types will result in a diagnostic error.
:::
