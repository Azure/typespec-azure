...
```csharp
// The C# emitter generates the model but does not apply the client default value.
...
```typescript
// The TypeScript emitter applies the client default values in the request serializer.
...
```java
// The Java emitter generates the model but does not apply the client default value.
...
```go
// The Go emitter generates the model with the default value mentioned in a comment,
// but does not apply it automatically.
...
```csharp
// The C# emitter generates the method but does not apply the client default value.
namespace TestService
{
  public partial class TestServiceClient
  {
    public TestServiceClient(Uri endpoint);
    public virtual Response<IReadOnlyList<Item>> GetItems(int? pageSize = default, string sortOrder = default, CancellationToken cancellationToken = default);
    public virtual Task<Response<IReadOnlyList<Item>>> GetItemsAsync(int? pageSize = default, string sortOrder = default, CancellationToken cancellationToken = default);
  }
}
```
...
```typescript
// The TypeScript emitter applies the client default values when building the request URL.
export interface GetItemsOptionalParams extends OperationOptions {
  pageSize?: number;
  sortOrder?: string;
}
...
export async function getItems(
  context: Client,
  options: GetItemsOptionalParams = { requestOptions: {} },
): Promise<Item[]>;
// Internally: expandUrlTemplate uses `options?.pageSize ?? 10` and `options?.sortOrder ?? "desc"`.
```
...
```java
// The Java emitter generates the method but does not apply the client default value.
public Response<BinaryData> getItemsWithResponse(RequestOptions requestOptions);
public List<Item> getItems(Integer pageSize, String sortOrder);
...
```go
// The Go emitter generates the optional-parameters struct with the default value mentioned
// in a comment, but does not apply it automatically.
type TestServiceClientGetItemsOptions struct {
...
func (client *TestServiceClient) GetItems(ctx context.Context, options *TestServiceClientGetItemsOptions) (TestServiceClientGetItemsResponse, error)
```
...
```java
// Java represents nullable reference types directly; the model and enum classes are
// generated under the `models` package.
