...
@alternateType(
  {
    identity: "externallib.ExternalModel",
    package: "externallib",
    minVersion: "1.0.0",
  },
  "python"
...
```python
# In Python SDK, instead of generating MyModel,
# the emitter will reference ExternalModel from externallib
from externallib import ExternalModel
```

```csharp
// In C# SDK, MyModel would be generated normally
// unless an external type is specified for C#
...
```typescript
// In TypeScript SDK, MyModel would be generated normally
// unless an external type is specified for TypeScript
...
```java
// In Java SDK, MyModel would be generated normally
// unless an external type is specified for Java
...
```go
// In Go SDK, MyModel would be generated normally
// unless an external type is specified for Go
type MyModel struct {
    Prop *string
}
```
...
The `ExternalType` model has the following properties:

- `identity` (required): The fully-qualified name of the external type (e.g., `pystac.Collection`).
- `package` (optional): The package that exports the external type (e.g., `pystac`).
- `minVersion` (optional): The minimum version of the package to use (e.g., `1.13.0`).

:::caution
External types (with `identity` property) cannot be applied to model properties. They must be applied to the type definition itself (Scalar, Model, Enum, or Union).
:::
...
- When `@alternateType` is applied to a `Scalar`, the alternate type must also be a `Scalar`.
- `@encode` on the original type will be overridden by the one defined in the alternate type.
...
The `@deserializeEmptyStringAsNull` decorator indicates that a model property of type `string` (or a scalar derived from `string`) should be deserialized as `null` when its value is an empty string (`""`). This is useful when a service returns empty strings to represent absent values.
...
```python
class MyModel(_model_base.Model):
    # When the service returns "", the value will be deserialized as None
...
```typescript
// When the service returns "", the value will be deserialized as undefined
export interface MyModel {
  displayName?: string;
  description?: string;
}
...
```go
// When the service returns "", the value will be deserialized as nil
type MyModel struct {
	DisplayName *string
	Description *string
}
