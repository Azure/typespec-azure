...
> **NOTE:** JS RLC is not in the business of customization. it will ignore client.tsp and the follow scenarios will not have impact on the JS RLC user experiences. In this context, TypeScript part means JS Modular Emitter.

## Default behavior

### Basic rules

By default, the namespace with `@service` decorator will be generated as a root client. The name for that client will be the namespace name concatenating `Client` as suffix.

Other sub namespaces and interfaces of each root client will be generated as operation groups with hierarchy.

The operation group sequence is determined by the order of the namespace declaration then the interface declaration.
...
By default, the operation group could only be initialized by root client or parent operation group.
...
The client and operation group sequence is determined by the order of the `@client` and `@operationGroup` decorators.
