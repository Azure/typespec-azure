...
@@clientInitialization(Storage, StorageMockClientOptions);
```

```python
from storage import StorageClient

client = StorageClient(blob_name="myBlobName", endpoint="<my-endpoint>", ...)
...
StorageClient client = new StorageClient().buildClient();
