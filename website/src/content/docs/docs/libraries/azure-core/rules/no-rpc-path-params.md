...
@route("/lrRpcOp")
op lrRpcOp is Azure.Core.LongRunningRpcOperation<{ @path foo: string }, {}, PollingStatus, StatusError>;
```
...
```tsp
@route("/analyze")
op analyze is RpcOperation<{ @body request: AnalysisRequest }, AnalysisResult>;
```
