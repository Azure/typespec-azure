...
`@client`'s `ClientOptions` now also accepts `scope` directly. If it disagrees with the legacy positional argument, TCGC reports a `conflicting-scope` warning and prefers the options bag value. See `design-docs/decorator-options-migration.md` for migration guidance and deprecation policy.
