...
The client namespace of each client will be the name of the namespace the interface belong to or the namespace itself.
...
You could use `@client` and `@operationGroup` to reconstruct the client hierarchy, but if any customizations did, client hierarchy will only be inferred from them. The logic defined in the default behaviors will not take affect anymore.

Also, when you reconstruct the client hierarchy, the client namespace of the client and operation group will follow the new namespace you defined in `client.tsp`. You could use `@clientNamespace` to override it.
...
### Renaming the single client name
...
### Renaming the single client namespace name
