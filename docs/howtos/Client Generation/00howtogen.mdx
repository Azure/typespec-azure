...
This page outlines the steps to build a client library based on your TypeSpec specification. It provides the guide of the whole process from initial API design to the release of SDKs. please visit https://aka.ms/azsdk/dpcodegen.
...
> **NOTE:** Generating all languages from the same folder may be complicated due to the preview status of the emitters and TypeSpec libraries, such as `typespec-azure-core` and `typespec-resource-manager`.
...
To simplify the process of getting your code to emit without the need for multiple command line options, it's best to modify your `tspconfig.yaml` file.
...
Several language repositories utilize the `tsp-client` tool to simplify the process of generating client libraries. For more information on the tool, see [Getting started with `tsp-client`](<./../Generating with tsp-client/tsp_client.md>).
