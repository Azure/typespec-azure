...
export type DemoServiceContext = Client & {
  path: {
    ...
    (path: "/analyze"): {
      ...
      } & RequestParameters): StreamableMethod<Analyze200Response | AnalyzeDefaultResponse>;
    };
    /** Resource for '/completions' has methods for the following verbs: post */
    (path: "/completions"): {
      post(options: {
        body: CompletionInput
      } & RequestParameters): StreamableMethod<Completions200Response | CompletionsDefaultResponse>;
    };
  };
};
```
...
export type DemoServiceContext = Client & {
  path: {
    ...
    (path: "/foo"): {
        ...
        } & RequestParameters): StreamableMethod<A200Response>;
        post(options?: {
          body?: {
            x: string
          }
        } & RequestParameters): StreamableMethod<B200Response>;
    };
  };
};
```
