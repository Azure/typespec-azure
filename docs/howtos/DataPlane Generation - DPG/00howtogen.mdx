...
Several language repositories support the `tsp-client` CLI to facilitate generating client libraries. For more information on the `tsp-client`, see [Getting started with `tsp-client`](./09tsp_client.mdx).
