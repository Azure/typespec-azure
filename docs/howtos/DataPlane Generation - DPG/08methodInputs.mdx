...
Resource create and update operations are not impacted by spread since they all have explict defined body parameter.
...
def scheduleRepairs(self, problem: str, contact: str, *, content_type: str = "application/json") -> RepairResult:
  ...
```
...
If you want client to keep the model, you could use a wrapper in TypeSpec to explict set the body to prevent spread.
