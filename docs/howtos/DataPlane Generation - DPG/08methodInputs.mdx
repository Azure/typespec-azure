...
1. `@overrideClientMethod`
2. If you don't want to do client customizations, you could use a wrapper to explicit set the body to prevent spread.
...
<TabItem value="Client Customizations" label="TypeSpec">

With the `@overrideClientMethod` decorator, you have the ability to override the default client method generated by TCGC from your service definition.

In the case of spread, if the service definition spreads a model onto the method signature, but you would like either all or some of the generated client SDKs to still generate with the model grouped together, this is where the `@overrideClientMethod` decorator comes in.

With `@overrideClientMethod`, you can also specify language `scope` to scope the changes to a specific language emitter.
...
@@overrideClientMethod(Widget.Service.scheduleRepairs, Widget.Client.scheduleRepairs);
```

</TabItem>

<TabItem value="`@bodyRoot`" label="TypeSpec">

If you do not want to use customizations in `client.tsp`, you can also add `@bodyRoot` onto the input in the `main.tsp` before you pass the model to the template.
