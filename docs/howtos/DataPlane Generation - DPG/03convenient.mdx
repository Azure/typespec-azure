...
```csharp
// If a model is input-only, there will be one public constructor with all required properties as parameter.
// And the required properties without setter. Optional properties will have both getter and setter
...
// If a model is output-only, there is no public constructor, and all properties only have getter, no setter.

// If a model is roundtrip (input + output), there will be one public constructor with all required properties as parameter.
// And all properties will have both getter and setter
namespace Azure.AI.OpenAI.Models
{
