...
You also shouldn't instantialize your own interface, instead you should call `createSdkContext` to create.
...
interface PythonSdkContext<TServiceOperation extends SdkServiceOperation> extends SdkContext<
...
function serializeClient<TServiceOperation extends SdkServiceOperation>(
  ...
  return {
    ...
    subClients: client.methods.filter(
      (x) => x.kind === "clientaccessor"
    ).map(
      (x) => serializeClient(sdkContext, x.response)
    ),
    methods: client.methods.filter(
      (x) => x.kind ===  "method"
    ).map(
      (x) => serializeServiceMethod(sdkContext, client, x)
    )
  }
}
...
```ts
function serializeServiceMethod<TServiceOperation extends SdkServiceOperation>(
  ...
  switch (method.kind) {
    case "basic":
      return serializeBasicServiceMethod(context, client, method);
    case "paging":
      return serializePagingServiceMethod(context, client, method);
    case "lro":
      return serializeLroServiceMethod(context, client, method);
    case "lropaging":
      return serializeLroPagingServiceMethod(context, client, method);
  }
}
```
...
```ts
function serializeBasicServiceMethod<TServiceOperation extends SdkServiceOperation>(
  context: PythonSdkContext<TServiceOperation>,
  client: SdkClientType<TServiceOperation>,
  ...
  return {
    ...
    operation: serializeServiceOperation(context, client, method.operation)
  },
}
```
...
```ts
export interface SdkPagingServiceMethod<TServiceOperation extends SdkServiceOperation> extends SdkServiceMethodBase<TServiceOperation> {
  kind: "paging";
...
>(
  ...
  return {
    ...method,
    name: camelToSnakeCase(method.name),
    operation: serializeServiceOperation(context, client, method),
    nextLinkOperation: method.nextLinkOperation ? serializeServiceOperation(context, client, method.nextLinkOperation) : undefined,
  },
}
```
...
```ts
export interface SdkLroServiceMethod<TServiceOperation extends SdkServiceOperation> extends SdkServiceMethodBase<TServiceOperation> {
  kind: "lro",
  ...
  initialOperation: TServiceOperation;
}
```
...
>(
  ...
  return {
    ...method,
    name: camelToSnakeCase(method.name),
    initialOperation: serializeServiceOperation(context, client, method),
  },
}
```
...
export interface SdkHttpOperation {
  ...
  // mapping of number to SdkHttpResponse for valid responses
  // we will use a string to represent a status code range
  responses: Record<number | string, SdkHttpResponse>;
  exceptions: Record<number | string, SdkHttpResponse>
}
```
...
```ts
function serializeServiceOperation<TServiceOperation extends SdkServiceOperation>(
  context: PythonSdkContext<TServiceOperation>,
  client: SdkClientType<TServiceOperation>,
  operation: TServiceOperation,
...
```ts
import { SdkServiceOperation, SdkContext } from "@azure-tools/typespec-client-generator-core";
import { PythonSdkType } from "./interfaces.js";
...
```ts
import { EmitContext } from "@typespec/compiler";
import { SdkContext, createSdkContext, SdkClientType } from "@azure-tools/typespec-client-generator-core";
...
>(
  ...
  return {
    ...
    parameters: client.initialization?.properties.map(
      (x) => serializeMethodParameter(sdkContext, x)
    ),
    subClients: client.methods.filter(
      (x) => x.kind === "clientaccessor"
    ).map(
...
export async function $onEmit(
  ...
  const pythonSdkPackage: PythonSdkPackage = {
    clients: sdkContext.clients.map(
      (x) => serializeClient(sdkContext, x)
    ),
    rootNamespace: sdkContext.sdkPackage.rootNamespace,
    models: sdkContext.sdkPackage.models.map(
      (x) => getPythonSdkType(sdkContext, x)
    ),
    enums: sdkContext.sdkPackage.enums.map(
      (x) => getPythonSdkType(sdkContext, x)
    )
  }
