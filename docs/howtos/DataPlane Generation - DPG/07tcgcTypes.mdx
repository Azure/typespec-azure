...
We currently only have examples based on the payload, so the examples model will be binded to the SdkServiceOperation.
...
For `SdkUnionExample`, since it is hard to determine whether the example value should belong to which union varient, we will keep the raw value and leave the work for the emitter.
