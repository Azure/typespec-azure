...
> {
  ...
  nameSpace: string;
  endpoint: string;
  // whether the endpoint params are part of a templatee endpoint or not
...
>(
  ...
  return {
    ...
    parameters: (
      ...
      client.initialization.properties.map(
...
>(
  ...
  // we should eventually expose some of these subclient accessors as
  // a public method on clients one day
...
function serializeMethodResponse<
  TServiceOperation extends SdkServiceOperation
>(
  context: PythonSdkContext<TServiceOperation>,
  response: SdkMethodResponse,
...
>(
  ...
  return {
    ...
    parameters: client.initialization?.properties.map(
      (x) => getSdkModelPropertyType(sdkContext, x)
    ),
    subClients: client.methods.filter(
      (x) => x.kind === "clientaccessor"
    ).map(
